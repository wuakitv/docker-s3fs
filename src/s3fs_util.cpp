...
time_t get_mtime(const char *str)
{
  ...
  if(str && '\0' != *str){
    strmtime = str;
    string::size_type colen_pos = strmtime.find(':', 0);

    if(string::npos != colen_pos) {
       struct tm tm;
 
       if (strptime(strmtime.c_str(),"%Y-%m-%dT%H:%M:%S", &tm) != NULL) {
         return mktime(&tm);
       }
       if (strptime(strmtime.c_str(),"%Y-%m-%d %H:%M:%S", &tm) != NULL) {
         return mktime(&tm);
       }
    }
    ...
    S3FS_PRN_INFO("%s", strmtime.c_str());
  }
